...
export default function NewStudentPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setIsDirty(false);
      router.replace('/dashboard/students');   } catch (error: any) {
  ...
  return (
    <div className="animate-in" style={{ display: 'flex', flexDirection: 'column', gap: '2rem', maxWidth: '850px', margin: '0 auto' }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: '1rem' }}>
        ...
        <div>
          <h1 style={{ fontSize: '1.75rem', fontWeight: 700, marginBottom: '0.25rem', color: 'var(--foreground)' }}>Enroll New Student</h1>
          <p style={{ color: 'var(--text-muted)' }}>Enter all required academic and personal details.</p>
        </div>
      </div>
      ...
      <form onChange={() => setIsDirty(true)} onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
            ...
            <div style={{ display: 'flex', gap: '1.5rem', alignItems: 'center' }}>
              <div 
                style={{ 
                  ...
                  background: photoPreview ? 'transparent' : 'var(--background)',
                  border: photoPreview ? 'none' : '1px dashed var(--border)',
              ...
              <div>
                ...
                >
                  <Camera size={14} /> {photoPreview ? "Change Photo" : "Upload / Take Photo"}
                </button>
              </div>
            </div>
            ...
            <div className="input-group">
              ...
              <div style={{ position: 'relative', display: 'flex', alignItems: 'center' }}>
                ...
                <input 
                  ...
                  onChange={e => setFormData({...formData, previousGrades: e.target.value})}
                />
              </div>
            </div>
          </div>
        ...
        <div className="animate-in delay-300" style={{ display: 'flex', justifyContent: 'flex-end', gap: '1rem', marginTop: '1rem', marginBottom: '2rem' }}>
          <Link href="/dashboard/students" className="btn btn-outline" style={{ textDecoration: 'none' }}>
            Cancel
          </Link>
