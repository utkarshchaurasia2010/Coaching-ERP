...
export default function EditBatchPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const loadData = async () => {
    try {
      ...
      const currentSubjects = batchSubjectsData ? batchSubjectsData.map(bs => bs.subject_id) : [];
      ...
      setError(err.message || err.details || "Failed to load batch data.");
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // 2. Update Batch Subjects (Delete existing, then insert new ones)
      ...
      setSuccess(true);
      setIsDirty(false);
      router.replace(`/dashboard/batches/${batchId}`);

    } catch (err: any) {
      console.error("Batch update error:", err);
      setError(err.message || err.details || "Failed to update batch.");
    } finally {
      setSaving(false);
    }
  };
    ...
    <div style={{ maxWidth: '800px', margin: '0 auto', paddingBottom: '3rem' }}>
      <div className="flex-between animate-in" style={{ marginBottom: '2rem' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '1rem' }}>
          ...
          <div>
            <h1 style={{ fontSize: '1.5rem', fontWeight: 700, color: 'var(--foreground)' }}>Edit Batch</h1>
            <p className="text-muted" style={{ fontSize: '0.875rem' }}>Update batch information and subjects.</p>
          </div>
        </div>
      </div>
